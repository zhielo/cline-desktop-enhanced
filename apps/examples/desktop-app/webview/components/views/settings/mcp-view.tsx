...
export function McpServersContent({
	...
	const configureIda = async () => {
		const serverName = "IDA idalib";
		setServerActionError(serverName);
		try {
			await upsertServer({
				name: serverName,
				previousName: servers.some((server) => server.name === serverName)
					? serverName
					: undefined,
				transportType: "stdio",
				command: "idalib-mcp",
				args: ["--stdio", "--max-workers", "1"],
				env: { IDA_MCP_MAX_WORKERS: "1" },
				disabled: false,
			});
		} catch (error) {
	...
	const headerActions = (
		<>
			...
			<Button
				variant="outline"
				size="sm"
				onClick={() => void configureIda()}
				disabled={busyServerName === "IDA idalib"}
				title="Configure an installed idalib-mcp server for persistent IDA analysis"
			>
				Configure IDA
			</Button>
			<Button
				...
				disabled={
					busyServerName === "Notion" || authorizingServerName === "Notion"
				}
				title="Connect the official hosted Notion MCP server with OAuth"
			>
				Connect Notion
			</Button>
