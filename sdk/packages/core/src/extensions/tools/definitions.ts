...
export function createReverseEngineeringTool(
	...
	return createTool<ReverseEngineeringInput, string>({
		...
		description:
			"Discover and use installed Ghidra, IDA, or JADX for authorized static analysis. Call discover without a target when capabilities are unknown. Use inspect before handling an unknown artifact, extract for safe ZIP/APK/JAR extraction, and analyze/decompile/script/open_gui for engine-backed work. Automatically use this tool when a binary, APK, archive, decompilation, disassembly, or reverse-engineering task requires it. Uses bounded output, timeouts, cancellation, private output directories, and archive resource ceilings. Does not bypass licensing.",
