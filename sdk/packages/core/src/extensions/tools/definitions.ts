...
export function createReverseEngineeringTool(
	executor: ReverseEngineeringExecutor,
): AgentTool<ReverseEngineeringInput, string> {
	return createTool<ReverseEngineeringInput, string>({
		name: "reverse_engineer",
		description:
			"Discover and use installed Ghidra, IDA, JADX, Apktool, Baksmali, Smali, Android SDK build tools, and Android Studio plugin directories for authorized static analysis. Choose the narrowest operation first: compare_apks when the user supplies original and modified APKs; inspect for fast structural triage; JADX decompile for readable Java and call-flow analysis; disassemble_smali plus search_smali/read_smali_method when code is obfuscated/protected or exact DEX instructions and complete method bodies matter; and Ghidra/IDA analyze or decompile for native .so libraries. compare_apks reports changed manifests, signatures, DEX, native libraries, resources, and assets without wasteful full extraction. search_smali performs a bounded indexed-style search with method context; read_smali_method lists methods or returns the complete .method through .end method block. Use extract only when raw files are required, assemble_smali/rebuild_apk for an editable round trip, and script/open_gui for advanced engine work. Call discover without a target when capabilities are unknown; it scans PATH, standard install roots, Android SDK roots, and Android Studio plugin locations. Analysis workspaces are cached by artifact hash and reused by default. JADX supports bounded threads, single-class work, deobfuscation, mappings, call graphs, JSON, fallback modes, and Gradle export. Automatically use this tool instead of ad-hoc shell scripts for binary, APK, archive, decompilation, disassembly, or reverse-engineering tasks. Uses per-workspace serialization, bounded output, timeouts, cancellation, private output directories, and archive resource ceilings. Does not execute target binaries or bypass licensing.",
...
export function createReadFilesTool(
	...
	return createTool<ReadFilesInput, ToolOperationResult[]>({
		...
		execute: async (input, context) => {
			...
			const results: ToolOperationResult[] = new Array(requests.length);
			let nextIndex = 0;
			const workers = Array.from(
				...
				async () => {
					while (nextIndex < requests.length) {
						...
						results[index] = await (async (): Promise<ToolOperationResult> => {
